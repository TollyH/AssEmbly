...
PROGRAM_START = [
    ...
    "; rg0, rg1, rg3, rg4 - temporary holding",
    "; rg2 - heap start address",
    "; rg9 - number of items on call stack",
    "MVQ rg2, :&WHITESPACE_HEAP_START",
    f"MVQ rg9, {MAX_STACK_VALUES}",
    "",
    "%ANALYZER warning, 0008, 0  ; Unreachable code detected",
    "%ANALYZER suggestion, 0018, 0  ; Label is defined but never used",
    "",
]
    ...
    for token in tokens:
        ...
        if len(parameter) > 0:
            ...
            for line in new_lines:
                # Evaluate the contents of curly brackets as Python code
                converted_program.append(FORMAT_VALUE_REGEX.sub(
                    lambda m: eval(
                        m[1], {"label": label, "number": number}
                    ), line
                ))
        else:
