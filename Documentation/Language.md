...
```text
IMP "file.asm"
INV  ; INV isn't a valid mnemonic, will result in an error
```
...
Assembling the first file will output the following error message:

```text
Error on line 5
    "INV"
Unrecognised mnemonic and operand combination:
    INV
Consult the language reference for a list of all valid mnemonic/operand combinations.
```

Note that the error message claims that the error is located on line 5, despite the fact that it is only line 2 in the original file. This has occurred because the `IMP` directive offset the line that contained the error, like so:
...
Attempting to assemble any of these three files would put the assembler into an infinite loop, as each file ends up depending on itself as it resolves its import.
